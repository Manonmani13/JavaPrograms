package com.proretention.medical.exception;
...
public class GlobalExceptionHandling {
	...
	public ResponseEntity<ErrorDetails> resourceNotFoundException(ResourceNotFound ex, WebRequest request) {
		ErrorDetails message = new ErrorDetails(HttpStatus.NOT_FOUND.value(), new Date(0), ex.getMessage(),
	...
	public ResponseEntity<ErrorDetails> globalExceptionHandler(Exception ex, WebRequest request) {
		ErrorDetails message = new ErrorDetails(HttpStatus.INTERNAL_SERVER_ERROR.value(), new Date(0), ex.getMessage(),
				request.getDescription(false));
